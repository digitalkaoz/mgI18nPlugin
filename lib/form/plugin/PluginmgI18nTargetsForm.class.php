...
class PluginmgI18nTargetsForm extends sfForm
{
  ...
  public function delete()
  {
    ...
    // initialize the delete query statement
    $delete_stm = $pdo->prepare("DELETE FROM trans_unit WHERE target = ? AND msg_id = ?");
    ...
    foreach($stm->fetchAll(PDO::FETCH_ASSOC) as $trans_unit)
    {
      ...
      $delete_stm->execute(array($target, $trans_unit['msg_id']));
